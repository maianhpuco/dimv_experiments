...
def svm_prediction(X_train, y_train, X_test, y_test, name, root, sub_folder):
    ...
    model = SGDClassifier(loss = 'modified_huber', n_jobs=6, max_iter = 5000)
    model.fit(X_train, y_train)
...
if __name__ == '__main__':
    root = '../../data/mnist/'
    accuracies = {}
    acc_path = '../../data/mnist/accuracy/v2_linear_svc.json'
    sub_folders = os.listdir(os.path.join(root, 'imputed/'))

    experiment_done = os.listdir("../../data/mnist/accuracy/v1/")
    count = 1 
    exps = [sub_folder for sub_folder in sub_folders \
            if not (len(sub_folder.split("_")) < 5) and \
                not (sub_folder in experiment_done) and \
                not (sub_folder.split("_")[1]=='70') and \
                not (sub_folder.split("_")[-1]=='20') ] 
