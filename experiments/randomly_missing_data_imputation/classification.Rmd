...
```{r}
PredictionOnEachFold <- function(fold, missing_data, data, labels, folds, dataset_name, folder_name, DIMVthreshold){ 
...
```{r}
ClassificationPipeline <- function(
    dataset, 
    dataset_name, 
    label_col, 
    DIMVthreshold, 
...
executeClassification <- function(root, MISSING_RATE, NUM_FOLDS, DATASETS){
...
```{r}
saved_root = "../../data/randomly_missing_dataset/svmRadial_20230116"
data_root = "../../data/randomly_missing_dataset/svmRadial_20230116/" 
...
DATASETS = c("iris", "ionosphere", "seeds", "wine")
...
for (DIMV_THRESHOLD in DIMV_THRESHOLD_LIST){
  for (MISSING_RATE in MISSING_RATE_LIST){
    ...
    print(">>>>>>>>>>>>>>START--------------")
    print(paste0("MISSING_RATE_", MISSING_RATE, "_THRESHOLD_" , "_NUM_FOLDS_", NUM_FOLDS))

    executeImputeAndClassification(root, MISSING_RATE, NUM_FOLDS, DATASETS)
