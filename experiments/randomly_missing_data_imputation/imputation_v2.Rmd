...
imputeAndPredictionOnEachFold <- function(fold, missing_data, data, labels, folds, dataset_name, DIMVthreshold){ 
  ...
  for(j in 1:length(func_list)){
    ...
    allNANinTrainRowsFound = length(which(rowSums(is.na(missing.X_train_normed))==dim(data)[2])) > 0 
    allNANinTestRowsFound = length(which(rowSums(is.na(missing.X_test_normed))==dim(data)[2])) > 0 
    
    if (allNANinTrainRowsFound || allNANinTestRowsFound){
      print(paste0("allNANinTrainRowsFound=", allNANinTrainRowsFound, "allNANinTestRowsFound_", allNANinTestRowsFound))
      stop("found all NAN in Rows in Train or Test, try to sample again")
    }
...
MISSING_RATE_LIST = c(.4, .5, .3,.2,.1,.6)
...
root = "../../data/randomly_missing_dataset/svmRadial_20230113_2"   
