...
```{r}
# imputed = impDi(S, missing.X_test_normed)
```

```{r}
imp = impDi(Sigma, missing.X_test_normed, 0.5)
``` 
...
impDi <- function(S, Xtest, threshold, nlargest=2){
  ...
  DIMV1feature <- function(f){
    ...
    allSameMissingPattern <- function(setF){
        return(sum(colSums(is.na(Xtest[naRows, setF, drop=F])) == length(naRows))==length(setF)) 
    }
    
    sameMissingPatternCheck = allSameMissingPattern(setF)
...
```
```{r}
imp = impDi(Sigma, X_train_normed, 0.5)
```
...
```
```{r}
impt = impDi(Sigma, missing.X_test_normed, 0.5)
```
...
```{r}
missingDataCreated = dataPreparation(
    0.6, 
    0.6, 
    0.5, 
    X.train, 
...
pth = '../../data/mnist/imputed_threhold_50/missing60pc50pcsample'

write.csv(result_impDi$train, file.path(curr_dir, pth, 'train_impDi.csv'), row.names=FALSE) 
write.csv(result_impDi$test, file.path(curr_dir, pth, 'test_impDi.csv'), row.names=FALSE) 
write.csv(result_softImpute$train, file.path(curr_dir, pth, 'train_softImpute.csv'), row.names=FALSE)
write.csv(result_softImpute$test, file.path(curr_dir, pth, 'test_softImpute.csv'), row.names=FALSE) 


write.csv(y_train, file.path(curr_dir, pth, 'y_train.csv'), row.names=FALSE)
write.csv(y_test, file.path(curr_dir, pth, 'y_test.csv'), row.names=FALSE)  
  

```

```{r}
visualize_digit(softImpute.Xrecon.train, y_train, train_removed_rows, 2, 8)
#
```

```{r}
visualize_digit(softImpute.Xrecon.test, y_test, test_removed_rows, 2, 5)
 
```
```{r}
visualize_digit(impDi.Xrecon.test, y_test, test_removed_rows, 2, 8)

```

```{r}
visualize_digit(impDi.Xrecon.train, y_train, train_removed_rows, 2, 8)
 
```


```{r}
pth = '../../data/mnist/imputed_threhold_50/missing40pc50pcsample'
...
```{r}
visualize_digit(softImpute.Xrecon.test , y_test, test_removed_rows, 2, 8)    
``` 
...
curr_dir = getwd()
pth = '../../data/mnist/imputed_threhold_30/missing40pc50pcsample'
...
write(y_train, file.path(curr_dir, pth, 'y_train.csv'))  
write(y_test, file.path(curr_dir, pth, 'y_test.csv'))  
``` 
...
```{r}
visualize_digit(impDi.Xrecon.test , y_test, test_removed_rows, 3, 10) 
```