...
impDi_run <- function(X.train, y.train, X.test, y.test, threshold=.1, print_progression=FALSE){ 
  #a) on training set 
  X.train[is.na(X.train)] <- NaN
  sigmaDper = dpers(X.train)
  if (print_progression==TRUE){
    print("Done computing covariance matrix, start imputing train set")
  }
  X_imp.train = impDi(sigmaDper, X.train, threshold=threshold, print_progression=print_progression)[,, drop=F]
  #b) on testing set   
  X.test[is.na(X.test)] <- NaN
  if (print_progression==TRUE){
    print("Done computing train set, start imputing test set")
  }
  X_imp.test = impDi(sigmaDper, X.test, threshold=threshold, print_progression=print_progression)[,, drop=F]
