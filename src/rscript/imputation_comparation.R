...
impDi_run <- function(X.train, y.train, X.test, y.test, threshold=.1){ 
  #a) on training set 
  ...
  # }
  X_imp.train = impDi(sigmaDper, X.train, threshold=threshold)[,, drop=F]
  #b) on testing set   
  X.test[is.na(X.test)] <- NaN
  # if (print_progression==TRUE){
  #   print("Done computing train set, start imputing test set")
  # }
  X_imp.test = impDi(sigmaDper, X.test, threshold=threshold)[,, drop=F]
...
MeanImp_run = function(X_train, y_train, X_test, y_test) {
	...
	X_train_imp = impute_mean(X_train);
